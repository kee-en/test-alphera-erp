$("#crew_filter_form").submit(function () {
	$.ajax({
		url: base_url + "onboarding-crew-search",
		type: "POST",
...
$("#BtnResetSearch").click(function () {
	$.ajax({
		url: base_url + "onboarding-search-reset",
		type: "POST",
		success: function (data) {
			location.reload(true);
		},
...
function onboardCrew(crew_code, monitor_code) {
	Swal.fire({
...
function PrintReport(c_type) {
	window.open(base_url + "print-onboarding" + "/" + c_type);
}
...
// 						}
// 					}
// 				} else if (data.actc_date != null || data.acis_date != null) {
// 					if (data.actc_date) {
// 						$("#mrsm_csd_0").prop("checked", true);
// 					}

// 					if (data.acis_date) {
// 						$("#mrsm_csd_1").prop("checked", true);
// 					}
// 				}

...
class Crew_promotion extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'crew-promotion');
        ...
        if ($conditions) {
            $total_crews = count($this->promotions->get_crew_promotion($conditions));
        } else {
            $total_crews = count($this->promotions->get_crew_promotion());
        }

        $this->load->library('pagination');

        $config['base_url'] = base_url('crew-promotion');
        ...
        if ($conditions) {
            $data['promotion'] = $this->promotions->get_crew_promotion($conditions);
        } else {
            $data['promotion'] = $this->promotions->get_crew_promotion();
        }
        ...
        $end_count = ($conditions['start'] + count($data['promotion']));
        ...
        $data['title_tag'] = "Crew Promotion | Alphera Marine Services, Inc.";
        ...
        $this->load->view('crew/crew_promotion');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }

    public function crew_promotion_search()
    {
    ...
    public function unset_crew_promotion_search()
    {
        $this->session->unset_userdata('cmp_name_search');
        $this->session->unset_userdata('vessel_search');
        $this->session->unset_userdata('status_search');
    ...
    public function get_mlc_for_promotion()
    {
        $crew_code = $this->input->post('code');
        $crew = $this->global->getCrew($crew_code);

        $result = $this->contracts->get_crew_mlc_for_promotion($crew['monitor_code']);
    ...
    public function promotion_list_report()
    {
        $result = $this->promotions->get_promotion_list();
        $positions = $this->promotions->get_position_promote();
        var_dump($result);
        $data['author'] = "Alphera Marine Services, Inc.";
        $data['title_tag'] = "Promotion List Report | Alphera Marine Services, Inc.";
        $data['meta_description'] = "";
        $data['promotion'] = $result;
        $data['prom_pos'] = $positions;

        $this->load->view('include/header', $data);
        $this->load->view('include/nav');
        $this->load->view('reports/promotion_list_report');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }
}

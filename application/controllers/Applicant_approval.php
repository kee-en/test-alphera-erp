...
class Applicant_approval extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->userdata('url') != 'for-approval') {
            $this->unset_search_approval();
        }
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'for-approval');
        ...
        if ($conditions) {
            $total_applicant = count($this->applicant_approval->getApplicantApproval($conditions));
        } else {
            $total_applicant = count($this->applicant_approval->getApplicantApproval());
        }

        $this->load->library('pagination');

        $config['base_url'] = base_url('for-approval');
        ...
        if ($conditions) {
            $data['applicants'] = $this->applicant_approval->getApplicantApproval($conditions);
        } else {
            $data['applicants'] = $this->applicant_approval->getApplicantApproval();
        }
        ...
        $data['title_tag'] = "Principal Approval | Alphera Marine Services, Inc.";
        ...
        $this->load->view('applicant/approval');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }

    public function search_approved()
    {
    ...
    public function unset_search_approval()
    {
        $this->session->unset_userdata('name_search');
        $this->session->unset_userdata('status_search');
        $this->session->unset_userdata('vessel_search');

...
class Applicant_failed extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->userdata('url') != 'not-qualified') {
            $this->unset_search_failed();
        }
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'not-qualified');
        ...
        if ($conditions) {
            $total_applicant = count($this->applicant_failed->getApplicantFailed($conditions));
        } else {
            $total_applicant = count($this->applicant_failed->getApplicantFailed());
        }

        $this->load->library('pagination');

        $config['base_url'] = base_url('not-qualified');
        ...
        if ($conditions) {
            $data['applicants'] = $this->applicant_failed->getApplicantFailed($conditions);
        } else {
            $data['applicants'] = $this->applicant_failed->getApplicantFailed();
        }
        ...
        $data['title_tag'] = "Not Qualified | Alphera Marine Services, Inc.";
        ...
        $this->load->view('applicant/failed');
    ...
    public function unset_search_failed()
    {
        $this->session->unset_userdata('name_search');
        $this->session->unset_userdata('status_search');
        $this->session->unset_userdata('vessel_search');

...
class For_onboarding extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->perPage = 30;
        if ($this->session->userdata('url') != 'for_onboarding') {
            $this->onboarding_search_reset();
        }
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'for_onboarding');
        ...
        if ($conditions) {
            $total_crews = count($this->onboarding->getOnboardingCrew($conditions));
        } else {
            $total_crews = count($this->onboarding->getOnboardingCrew());
        }

        $this->load->library('pagination');

        $config['base_url'] = base_url('for_onboarding');
        ...
        if ($conditions) {
            $data['crew'] = $this->onboarding->getOnboardingCrew($conditions);
        } else {
            $data['crew'] = $this->onboarding->getOnboardingCrew();
        }
        ...
        $data['title_tag'] = "For Onboarding | Alphera Marine Services, Inc.";
        ...
        $this->load->view('crew/onboarding');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }

    public function onboarding_crew_search()
    {
    ...
    function onboarding_search_reset()
    {
        $this->session->unset_userdata('cmp_name_search');
        $this->session->unset_userdata('vessel_search');
        $this->session->unset_userdata('rank_search');
    ...
    public function add_crew_cmplan()
    {
        $return_update_contracts = $this->contracts->updateContractOngoing();

        if ($return_update_contracts === true) {

            $result = $this->onboarding->addCrewCMPlan();
            ...
            if ($result) {
                ...
                $data = [
                    'type' => 'error',
                    'title' => 'Save Failed.',
                    'title' => 'Cannot be added to CM Plan, missing requirements try adding flight details or contract.'
                ];
            } else {
                $data = [
                    'type' => 'error',
    ...
    public function add_crew_flight()
    {
        $flight_code = $this->input->post('flight_code');
        $monitor_code = $this->input->post('monitor_code');

        $result = $this->onboarding->addCrewFlight($flight_code, $monitor_code);
        ...
        if ($result === true) {
            ...
            $data = [
                'type' => 'error',
                'title' => 'There are no changes with your data!'
            ];
        }

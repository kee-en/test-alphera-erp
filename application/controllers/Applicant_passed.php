...
class Applicant_passed extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->userdata('url') != 'passed-applicants') {
            $this->unset_search_passed();
        }
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'passed-applicants');
        ...
        if ($conditions) {
            $total_applicant = count($this->applicant_passed->getApplicantPassed($conditions));
        } else {
            $total_applicant = count($this->applicant_passed->getApplicantPassed());
        }

        $this->load->library('pagination');

        $config['base_url'] = base_url('passed-applicants');
        ...
        if ($conditions) {
            $data['applicants'] = $this->applicant_passed->getApplicantPassed($conditions);
        } else {
            $data['applicants'] = $this->applicant_passed->getApplicantPassed();
        }
        ...
        $data['title_tag'] = "Passed Applicants | Alphera Marine Services, Inc.";
        ...
        $this->load->view('applicant/passed');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }

    public function search_passed()
    {
    ...
    public function unset_search_passed()
    {
        $this->session->unset_userdata('name_search');
        $this->session->unset_userdata('status_search');
        $this->session->unset_userdata('vessel_search');
    ...
    public function move_cm_plan()
    {
        $applicant_code = $this->input->post('applicant_code');

        $result = $this->applicant_passed->moveCMPlan($applicant_code);

        if ($result === true) {
            $data = [
                'type' => 'success',
                'title' => 'Applicant moved to Crew Management Plan'
            ];
        } else {
            $data = [
                'type' => 'error',

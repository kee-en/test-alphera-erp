...
class Pre_joining_visa extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->userdata('url') != 'pre-joining-visa') {
            $this->prejoining_search_reset();
        }
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'pre-joining-visa');
        ...
        if ($conditions) {
            $total_crew = count($this->pre_joining->getPreJoiningCrew($conditions));
        } else {
            $total_crew = count($this->pre_joining->getPreJoiningCrew());
        }

        $this->load->library('Pagination');

        $config['base_url'] = base_url('pre-joining-visa');
        ...
        if ($conditions) {
            $data['crew'] = $this->pre_joining->getPreJoiningCrew($conditions);
        } else {
            $data['crew'] = $this->pre_joining->getPreJoiningCrew();
        }
        ...
        $data['title_tag'] = "Prejoining & Visa | Alphera Marine Services, Inc.";
        ...
        $this->load->view('crew/crew_monitoring/pre_joining_visa');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }

    public function save_crew_licenses()
    {
        $result = $this->pre_joining->saveCrewLicenses();
    ...
    public function save_crew_trainings()
    {
        $result = $this->pre_joining->saveCrewTrainings();
    ...
    function prejoining_search_reset()
    {
        $this->session->unset_userdata('cmp_name_search');
        $this->session->unset_userdata('vessel_search');
        $this->session->unset_userdata('rank_search');

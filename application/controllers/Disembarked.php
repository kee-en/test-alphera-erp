...
class Disembarked extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->userdata('url') != 'crew-disembarked') {
            $this->disembarked_search_reset();
        }
    }
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('url', 'crew-disembarked');
        ...
        if ($conditions) {
            $total_crews = count($this->disembark->getCrewDisembark($conditions));
        } else {
            $total_crews = count($this->disembark->getCrewDisembark());
        }

        $this->load->library('pagination');

        $config['base_url'] = base_url('crew-disembarked');
        ...
        if ($conditions) {
            $data['crew'] = $this->disembark->getCrewDisembark($conditions);
        } else {
            $data['crew'] = $this->disembark->getCrewDisembark();
        }
        ...
        $data['title_tag'] = "Disembarked | Alphera Marine Services, Inc.";
        ...
        $this->load->view('modal/crew/slip/disembarked_routing_slip');
        $this->load->view('crew/disembarked');
        $this->load->view('include/footer');
        $this->load->view('include/script');
    }

    public function disembarked_crew_search()
    {
    ...
    function disembarked_search_reset()
    {
        $this->session->unset_userdata('cmp_name_search');
        $this->session->unset_userdata('vessel_search');
        $this->session->unset_userdata('rank_search');
